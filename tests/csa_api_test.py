...
class ApiTests(unittest.TestCase):
    ...
    def test_connect_with_HTTP_auth(self):
        responses.add(responses.GET,
                      self.api._build_end_point_uri('/users/show/:id', {':id': '42'}),
                      body=load_fixture('users/show/42.json'),
                      status=200,
                      content_type='application/json')

        self.api = CsaAPI("admin", "taliesin")
        resp = self.api.make_request("/users/show/:id", {':id':'42'})
        nose.tools.assert_equal("Jackson", resp["surname"])
